...
echo -----------------------------------------------------
echo         Installing KB4012212 HF (wannacry)
echo                 %date%  -  %time%
echo -----------------------------------------------------
...
robocopy D:\ADM\Wannacry\HF  \\%%a\c$\hotfix /MIR /R:0 /W:0

psexec -s -u domain\admin_user -p password -h \\%%a wusa "\\%%a\c$\hotfix\kb4012212.msu" /quiet /norestart >> detail_%MYDATE: =%.log 2>&1

REM :: % We use two echo commands, the first one send the output to STDOUT and the second one to a log file %
echo ##### Installing kb4012212-x86.msu Computer %%a #####  exit code: !errorlevel!
echo ##### Installing kb4012212-x86.msu Computer %%a #####  exit code: !errorlevel! >> log.txt 2>&1

REM :: % Deleting folder %
echo Deleting folder
rmdir \\%%a\c$\hotfix /S /Q
)
