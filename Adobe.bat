REM :: % The loop will read the computer names from a file (computers.txt) executing the installation command line for each one. %
...
psexec \\%%a -c -s -u domain\admin_user -p password \\path\where\Software\is\located\AcroRd.exe /sAll /rs  >> detail_%MYDATE: =%.log 2>&1

REM :: % We use two echo commands, the first one send the output to STDOUT and the second one to a log file %
echo ##### Installing Adobe Acrobat Computer %%a #####  exit code: !errorlevel!
echo ##### Installing Adobe Acrobat Computer %%a #####  exit code: !errorlevel! >> log.txt 2>&1
) 
