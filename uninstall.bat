...
REM :: % The Key between Curly brackets {} is the UninstallString value in windows register%
REM :: % This is an example for java X86_8u91 %
psexec \\%%a -s -u domain\admin_user -p password MsiExec.exe /qn /X{26A24AE4-039D-4CA4-87B4-2F83218091F0} /norestart >> detail_%MYDATE: =%.log 2>&1

REM :: % We use two echo commands, the first one send the output to STDOUT and the second one to a log file %
echo ##### Uninstalling XXXX Computer %%a #####  exit code: !errorlevel!
echo ##### Uninstalling XXXX Computer %%a #####  exit code: !errorlevel! >>log.txt 2>&1
