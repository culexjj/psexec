...
for /F "tokens=*" %%a in (computers.txt) do (
	...
	if !errorlevel! EQU 0 ( 		
		REM :: % copying files to target computer %
		xcopy .\sources  \\%%a\c$\temp\fonts /Y /S /I
		psexec.exe -s -u %user% -p %pass% D:\ADM\Sources\firefoxsetup.exe -ms >> .\Logs\detail_%MYDATE: =%.log 2>&1
		echo %%a,!errorlevel! >> log.txt 2>&1
		echo ##### Computer %%a updated  #####
		REM :: % Deleting folder %
		rmdir \\%%a\c$\temp\fonts /S /Q
	) else (
