...
psexec \\%%a -c -s -u domain\admin_user -p password \\path\where\Software\is\located\flashplayer.exe -install -force >> detail_%MYDATE: =%.log 2>&1

REM :: % We use two echo commands, the first one send the output to STDOUT and the second one to a log file %
echo ##### Installing Plugin  Computer %%a #####  exit code: !errorlevel!
echo ##### Installing Plugin  Computer %%a #####  exit code: !errorlevel! >>log.txt 2>&1
) 
